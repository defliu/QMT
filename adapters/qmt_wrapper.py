...
class Trader:
    ...
    def sell(self, stock_code, volume, remark='', use_market=False):
        ...
        # 重试/撤单后重挂：直接市价确保成交
        if use_market:
            return self._passorder(self.SELL_CODE, stock_code, vol, remark, price_type=5, price=-1)
        ...
        return self._passorder(self.SELL_CODE, stock_code, vol, remark, price_type=0, price=sell_price)
    ...
    def get_total_asset(self):
        try:
            accounts = get_trade_detail_data(self.acct, self.acct_type, 'account')
            if accounts:
                # xtquant版本差异：属性名可能是 m_dTotalAsset 或 totalAsset
                acct = accounts[0]
                # 先尝试完整资产字段
                for attr in ('m_dTotalAsset', 'totalAsset', 'm_dTotal'):
                    val = getattr(acct, attr, None)
                    if val is not None:
                        return float(val)
                # 回退：市值 + 可用资金
                mv = float(getattr(acct, 'm_dMarketValue', 0) or 0)
