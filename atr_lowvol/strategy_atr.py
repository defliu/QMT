    ...
    for code, info in list(_g_my_codes.items()):
        ...
        if _ENABLE_CONDITION_EXIT:
            ...
            if df is not None and len(df) >= _MIN_BARS:
                ...
                if atr_pct >= _ATR_THRESHOLD or turnover < _MIN_TURNOVER or turnover > _MAX_TURNOVER:
                    to_sell.append((code, '条件失效 ATR%%=%.2f 换手=%.2f%%' % (atr_pct, turnover)))
                    continue
...
def _execute_buys(C, candidates):
    ...
    budget = total_asset - holdings_value
    current_count = len(_g_my_codes)
    available_slots = _MAX_HOLD - current_count

    if available_slots <= 0 or budget <= 0:
        print("  [ATR买入] 无可用仓位或资金")
        return

    # 等分资金
    per_slot_budget = budget * _TARGET_RATIO / available_slots
