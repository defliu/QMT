...
def run_backtest(
    ...
    """T003: 阈值过滤器回测(双向). direction: 'top'(TopN原版)/'ge'(≥threshold)/'le'(≤threshold逆向).
    apply_cost: 单边滑点0.5%+佣金万1+印花税千1(卖)."""
    """
        ...
        if len(portfolio) < max_hold:
            ...
            for code6, df in all_data.items():
                ...
                try:
                    ...
                    if _calc_buy_bias5(hist) > MAX_BUY_BIAS5:
                        continue
            ...
            if today_scores:
                # T003: 阈值过滤(direction=ge/le) vs TopN(direction=top)
                if direction == 'ge':
                    picks = [(c, s) for c, s in today_scores if s >= threshold]
                elif direction == 'le':
                    picks = [(c, s) for c, s in today_scores if s <= threshold]
                else:
...
def parse_args(argv=None):
    ...
    parser.add_argument('--threshold', type=float, default=0.0,
                        help='Score threshold for ge/le filter (default 0=TopN)')
    parser.add_argument('--direction', type=str, default='top', choices=['top', 'ge', 'le'],
                        help='Selection: top(TopN原版)/ge(>=threshold)/le(<=threshold逆向)')
    parser.add_argument('--tag', type=str, default='', help='输出标签')
...
def main():
    ...
    tag = args.tag or f"{args.direction}_{args.threshold}"
    print(f"\n===T003_RESULT|{tag}|trades={m.get('total_trades')}|ret={m.get('total_return')}|dd={m.get('max_drawdown')}|sharpe={m.get('sharpe_ratio')}|win={m.get('win_rate')}===")
