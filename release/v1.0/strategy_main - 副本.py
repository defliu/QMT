...
class Trader:
    ...
    def sell(self, stock_code, volume, remark=''):
        """
        限价卖出：获取卖一价挂单，失败时回退市价单
        """
        if SAFEMODE_ENABLED:
            price = 0.0
...
def _check_and_execute_sell(C, today):
    global _g_sell_engine, _g_pending_sells
    ...
    for code, dec, shares in raw_decisions:
        ...
        if order_id:
            ...
            print("    [卖出委托失败] %s %d股  原因=%s" % (code, shares, dec.reason))
...
def _check_pending_sells(C, today):
    ...
    for code, info in list(_g_pending_sells.items()):
        ...
        if vol_traded >= info['volume']:
            ...
            elif info.get('retries', 0) >= 3:
                print("  [卖出放弃] %s 重试%d次仍未成交，放弃卖出" % (code, info.get('retries', 0)))
                _g_pending_sells.pop(code, None)
            else:
...
def _retry_pending_sell(code, info, C):
    ...
    order_id = _g_trader.sell(code, new_vol, remark='卖出')
    if order_id:
        _g_pending_sells[code] = dict(
            info,
            order_id=order_id,
        ...
        print("    -> 委托失败 %s %d股，下一分钟再试" % (code, new_vol))
...
def _try_retry_queue(C):
    ...
    for item in _g_retry_queue:
        code = item['code']
        if _g_sell_engine and not _g_sell_engine.is_reentry_allowed(code, today, _g_all_data.get(code)):
            print("  [重试禁入] %s 仍在20日禁入期，放弃重试" % code)
            _try_buy_replacement(C, '重试禁入补买')
            continue

        if item['retries'] >= 3:
            print("  [重试放弃] %s 重试%d次失败" % (code, item['retries']))
...
def _check_pending_orders(C):
    ...
    for code, info in list(_g_pending_buys.items()):
        ...
        if vol_traded > 0:
            ...
            _g_trader.cancel_order(info['order_id'], code)
            print("  [重试用完] %s 3次撤单重试未成交，顺位替补..." % code)
            _try_buy_replacement(C, '盘中')
        else:
            _g_trader.cancel_order(info['order_id'], code)
            ...
            if new_price and new_price > 0:
                ...
                if new_vol >= 100:
                    ...
                    if new_id is not None:
                        ...
                        print("    -> 重试委托失败，加入重试队列(累计重试%d次)" % retry_count)
                        _g_retry_queue.append({
                            'code': code,
                            'amount': _g_per_stock_amount,
                            'retries': retry_count,
...
def _all_day_decision_matrix(C, today, dt, scored_candidates, held_scores):
    ...
    for code in list(_g_my_codes.keys()):
        ...
        # 卖出引擎评估
        decision = _g_sell_engine.evaluate(code=code, df=df, today=today)
        ...
        if decision.action == Action.REDUCE:
            ...
            if code in held_scores and held_scores[code] < 70:
                ...
                if cand_score and cand_score['score'] >= held_scores[code] + 15:
                    _execute_swap(code, cand_score, decision, C)
                    continue
            # 否则执行减仓
            _check_and_execute_sell(C, today)
            continue
...
class StrategyRunner(object):
    ...
    def handlebar(self, C):
        ...
        if DEBUG_MODE or TEST_MODE or now >= '0930':
            if now < '1458':
        ...
        if _g_pending_buys or _g_retry_queue:
            ...
            if TEST_MODE:
                _g_today_done = True
            elif not _g_pending_buys and not _g_retry_queue and not _g_pending_sells:
                _g_today_done = True
