...
def _get_all_d_concept_sectors(C):
    ...
    try:
        ...
        print("  [板块] QMT 返回 %d 个板块" % total)

        # 诊断：打印前50个板块名称及前缀分布
        sample = all_sectors[:50] if total > 50 else all_sectors
        print("  [板块] 全部板块名称 (前%d):" % len(sample))
        for i, sec in enumerate(sample):
            print("    [%d] %s" % (i, sec))

        prefixes = {}
        for sec in all_sectors:
            pfx = sec[:4] if len(sec) >= 4 else sec
            prefixes[pfx] = prefixes.get(pfx, 0) + 1
        sorted_pfx = sorted(prefixes.items(), key=lambda x: -x[1])
        print("  [板块] 板块名称前缀分布 (前20):")
