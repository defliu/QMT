...
def init(C):
    ...
    print("[MF] 本金: %d元 | 单票上限: %.0f%% | 止损: %.0f%%" % (CAPITAL, MAX_WEIGHT*100, STOP_LOSS*100))
    print("[MF] 调仓: 双月 | TOP%d | 市值: 0-30亿 | 成交额>2000万" % TOP_N)
    print("[MF] 持仓文件: %s" % POSITIONS_FILE)
    print("[MF] =============================================")
...
def handlebar(C):
            ...
            if tick and code in tick:
                ...
                if last_price > 0 and entry_price > 0:
                    ...
                    if ret <= STOP_LOSS:
                        ...
                        print("[MF] 止损触发 %s: 入场%.2f 现价%.2f 跌幅%.1f%%" % (
    ...
    for code, shares in to_sell:
        try:
            ...
            print("[MF] 止损卖出 %s %d股" % (code, shares))
            if code in positions:
                del positions[code]
        except Exception as e:
            print("[MF] 止损卖出失败 %s: %s" % (code, e))
    ...
    for code in md:
        ...
        df_list.append(code)
    valid_codes = df_list
    print("[MF] 有效股票: %d只" % len(valid_codes))
    ...
    for code in valid_codes:
        try:
            ...
            continue
    print("[MF] 过滤后候选: %d只" % len(candidates))
    ...
    if len(scores) < TOP_N:
        print("[MF] 评分不足TOP%d，实际%d只" % (TOP_N, len(scores)))
        selected = scores.index.tolist()
    else:
        selected = scores.head(TOP_N).index.tolist()

    print("[MF] 选中%d只, 最高分%.1f, 最低分%.1f" % (
    ...
    for code in list(held_codes):
        if code not in selected_set:
            ...
            if shares > 0:
                try:
                    C.passorder(24, 1101, C.accountid, code, 5, -1, shares, C, strRemark="mfic")
                    print("[MF] 卖出 %s %d股" % (code, shares))
                except Exception as e:
                    print("[MF] 卖出失败 %s: %s" % (code, e))
            if code in positions:
                del positions[code]
    ...
    for code in selected:
        ...
        try:
            ...
            print("[MF] 买入 %s %.2f×%d股=%.0f元" % (code, price, volume, price*volume))
            positions[code] = {"shares": volume, "entry_price": price, "buy_date": today_str}
        except Exception as e:
            print("[MF] 买入失败 %s: %s" % (code, e))
